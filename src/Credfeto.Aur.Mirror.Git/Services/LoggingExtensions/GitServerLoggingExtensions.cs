...
internal static partial class GitServerLoggingExtensions
{
    [LoggerMessage(
        ...
        EventId = 1,
        Message = "Executing Git command: {arguments}"
    )]
    ...
    public static partial void FailedToStartGit(
    ...
    [LoggerMessage(
        LogLevel.Information,
        EventId = 5,
        Message = "Failed to clone {upstream} into {path}: {message}"
    )]
    ...
    [LoggerMessage(
        LogLevel.Information,
        EventId = 6,
        Message = "Failed to update {upstream} in {path}: {message}"
    )]
    ...
    public static partial void RepoMissing(
        this ILogger<GitServer> logger,
        string repo,
        string path
    );
    ...
    [LoggerMessage(
        LogLevel.Information,
        EventId = 9,
        Message = "{repo}: Failed to open {path}: {message}"
    )]
    public static partial void RepoInvalid(
        this ILogger<GitServer> logger,
        string repo,
        string path,
        string message,
