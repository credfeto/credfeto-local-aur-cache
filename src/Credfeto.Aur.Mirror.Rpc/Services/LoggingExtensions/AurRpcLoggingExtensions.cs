...
internal static partial class AurRpcLoggingExtensions
{
    ...
    [LoggerMessage(LogLevel.Information, EventId = 6, Message = "Package {package} was not accessed recently (Accessed {lastAccessed} was {age} ago Max Age {maxAge})")]
    public static partial void NotAccessedRecently(this ILogger<AurRpc> logger, string package, DateTimeOffset lastAccessed, TimeSpan age, TimeSpan maxAge);

    [LoggerMessage(LogLevel.Information, EventId = 7, Message = "Package {package} was not requested recently (Requested {lastRequested} was {age} ago Max Age {maxAge})")]
    public static partial void NotRequestedRecently(this ILogger<AurRpc> logger, string package, DateTimeOffset lastRequested, TimeSpan age, TimeSpan maxAge);
