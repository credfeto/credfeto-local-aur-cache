...
internal static partial class LocalAurRpcLoggingExtensions
{
    ...
    public static partial void RequestingUpstream(this ILogger<LocalAurRpc> logger, Uri uri);

    [LoggerMessage(LogLevel.Information, EventId = 2, Message = "Requesting from upstream {uri} => {statusCode}")]
    public static partial void SuccessFromUpstream(this ILogger<LocalAurRpc> logger, Uri uri, HttpStatusCode statusCode);
    ...
    public static partial void Query(this ILogger<LocalAurRpc> logger, string query);

    [LoggerMessage(LogLevel.Error, EventId = 9, Message = "Failed query: {query}  => {message}")]
    public static partial void Failed(this ILogger<LocalAurRpc> logger, string query, string message, Exception exception);

    [LoggerMessage(LogLevel.Information, EventId = 10, Message = "Searching for: {keyword} by {by}")]
    public static partial void SearchingFor(this ILogger<LocalAurRpc> logger, string keyword, string by);

    [LoggerMessage(LogLevel.Information, EventId = 11, Message = "Retrieving package infos: {packages}")]
    private static partial void PackageInfo(this ILogger<LocalAurRpc> logger, string packages);

    public static void PackageInfo(this ILogger<LocalAurRpc> logger, IReadOnlyList<string> packages)
    {
